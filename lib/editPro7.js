...
async function mergeLanguages(paths) {
  ...
  let maxNumCues = Math.max(presentations[0].cues.length, presentations[1].cues.length);

  for (let cue = 0; cue < maxNumCues; cue++) { // forEach doesn't consider await, use for instead
    // Find action in current cue of second presentation that's a slide
