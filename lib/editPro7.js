  ...
  presentations.forEach((presentation, i) => {
    presentation.cueGroups.forEach(cueGroup => {
      cueGroup.cueIdentifiers.forEach(id => {
        let index = presentation.cues.findIndex(cue => cue.uuid.string === id.string);
        cueIds[i].push(index);
      });
    });
  });

  for (let cue = 0; cue < Math.min(cueIds[0].length, cueIds[1].length); cue++) { // forEach doesn't consider await, use for instead
    // Get action in current cue of second presentation that's a slide
    let action = presentations[1].cues[cueIds[1][cue]].actions.find(action => action.type === 'ACTION_TYPE_PRESENTATION_SLIDE');
    ...
    let index = presentations[0].cues[cueIds[0][cue]].actions.findIndex(action => action.type === 'ACTION_TYPE_PRESENTATION_SLIDE');

    // Add first element of action to current cue of first presentation
    if (presentations[0].cues[cueIds[0][cue]].actions[index].slide.presentation.baseSlide.elements !== undefined) {
      presentations[0].cues[cueIds[0][cue]].actions[index].slide.presentation.baseSlide.elements.push(action.slide.presentation.baseSlide.elements[0]);
    } else {
      presentations[0].cues[cueIds[0][cue]].actions[index].slide.presentation.baseSlide.elements = action.slide.presentation.baseSlide.elements;
    }
  }
...
function readFile(pathToFile) {
  return new Promise(resolve => {
    ...
    protobuf.load(__dirname + '../plugins/ProPresenter7-Proto/proto/presentation.proto', function(error, root) {
