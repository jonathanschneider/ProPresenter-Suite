...
function mergeLanguages(pathToFile) {
  return new Promise(async (resolve, reject) => {
    var presentation = await readFile(pathToFile);

    writeFile(pathToFile + ' (merged)', presentation);
    resolve('Merged languages in ' + path.basename(pathToFile));
  });
}

function switchLanguages(pathToFile) {
  return new Promise(async (resolve, reject) => {
    var presentation = await readFile(pathToFile);
    ...
    // Write to new file
    writeFile(pathToFile.replace('.pro', ' (switched).pro'), presentation);
    resolve('Switched languages in ' + path.basename(pathToFile));
  });
}
...
function writeFile(pathToFile, object) {
  return new Promise(resolve => {
    ...
    fs.writeFile(path.resolve(pathToFile), buffer, function(error) {
      if (error) throw error;
      log(path.basename(pathToFile) + ' saved');
      resolve();
    });
  });
}
